...
void		julia(t_fractol *data)
{
	...
	a.k = i_comp(
		1 * ((float)data->x_jul / data->w - 0.5),
		1 * ((float)(data->h - data->y_jul) / data->h - 0.5));
	julia2(data, &a, xy, it_m);
}

void		julia2(t_fractol *data, t_complex_all *a, int xy[2], int it_m[2])
{
	it_m[1] = 50;
	xy[1] = -1;
	while (++xy[1] < data->h)
	{
		...
		while (++xy[0] < data->w)
		{
			...
			while (pow(a->z.re, 2.0) + pow(a->z.im, 2.0) <= 4
				&& it_m[0] < it_m[1])
			{
				a->z = i_comp(pow(a->z.re, 2.0) - pow(a->z.im, 2.0) + a->k.re,
					2.0 * a->z.re * a->z.im + a->k.im);
				it_m[0]++;
			}
